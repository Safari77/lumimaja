...
bool CXMLprefs::CreateXML(bool bLoad)
{
  ...
  if (!bLoad && m_pXMLDoc != NULL) {
    ...
    pugi::xml_node node = m_pXMLDoc->append_child(_T("Lumimaja_Settings"));
    return node != NULL;
  } else
                            ...
                            std::vector<st_prefShortcut> v_shortcuts)
{
  ...
  // csBaseKeyName is "Lumimaja_Settings\host\user\Shortcuts"
  // Even if this does not exist, "Lumimaja_Settings\host\user" must so we can just go
                                ...
                                const stringT &sHost, const stringT &sUser)
{
  ...
  pugi::xml_node root_node = m_pXMLDoc->first_element_by_path(_T("Lumimaja_Settings"), _T('\\'));
  if (root_node == NULL)
    return false;
                                       ...
                                       bool &bNoMoreNodes)
{
  ...
  stringT sPath = stringT(_T("Lumimaja_Settings//")) + sHost;
  pugi::xml_node node = m_pXMLDoc->first_element_by_path(sPath.c_str(), _T('\\'));
