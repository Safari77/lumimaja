...
{
  // LUMIMAJA_PREFSDIR overrides all:
  stringT retval(pws_os::getenv("LUMIMAJA_PREFSDIR", true));
  if (retval.empty()) {
    retval = pws_os::getuserprefsdir(); // "LOCAL_APPDATA\Lumimaja" for Windows,
                                        // ~/.config/lumimaja for Linux
    if (retval.empty())
