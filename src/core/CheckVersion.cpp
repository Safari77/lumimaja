 ...
 * And is of the form:
 * <VersionInfo>
 *  <Product name="Lumimaja" variant="PC" major="3" minor="28" build="0" rev="4786" />
 *  <Product name="Lumimaja" variant="PPc" major="1" minor="9" build="2" rev="100" />
 *  <Product name="Lumimaja" variant="U3" major="3" minor="28" build="0" rev="4786" />
 *  <Product name="Lumimaja" variant="Linux" major="0" minor="7" build="0" rev="4527:4532" />
 * </VersionInfo>
 *
...
CheckVersion::CheckLatestVersion(const stringT &xml, stringT &latest) const
{
  ...
  for (pugi::xml_node_iterator it = Root.begin(); it != Root.end(); ++it) {
    if (SafeCompare(it->name(), _T("Product"))) {
      ...
      if (SafeCompare(prodName, _T("Lumimaja"))) {
        ...
        if ((SysInfo::IsUnderU3()  && variant == _T("U3"))    ||
            ...
            if ((xmajor > m_nMajor) ||
                ...
                if (xbuild == 0) { // hide build # if zero (formal release)
                  Format(latest, L"Lumimaja V%d.%02d (%d)",
                         xmajor, xminor, xrevision);
                } else {
                  Format(latest, L"Lumimaja V%d.%02d.%02d (%d)",
