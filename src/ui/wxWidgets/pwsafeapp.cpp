...
bool PwsafeApp::OnInit()
{
  ...
  if (!prefs->GetPref(PWSprefs::MultipleInstances) &&
        (appInstance.Create(wxT("lumimaja.lck"), towxstring(pws_os::getuserprefsdir())) &&
         appInstance.IsAnotherRunning()))
  {
    wxMessageBox(_("Another instance of Lumimaja is already running"), _("Lumimaja"),
  ...
  if (cmd_silent) {
    if ( wxTaskBarIcon::IsAvailable() ) {
      ...
      // Note that if UseSystemTray is already true, then lumimaja will try to run silently anyway
      PWSprefs::GetInstance()->SetPref(PWSprefs::UseSystemTray, true);
    }
    else {
      // We don't want to bring up a UI if running silently
      std::wcerr << L"There appears to be no system tray support in your current environment.  "
                     "lumimaja may not work as expected in silent mode." << std::endl;
    }
  }
  ...
  if (!m_helpController){ // helpController (re)created  on language activation
    ...
    if (!prefs->GetPref(PWSprefs::IgnoreHelpLoadError) && !cmd_silent) {
      ...
      wxRichMessageDialog dlg(NULL,
        ...
        _("Lumimaja: Error initializing help"), wxCENTRE|wxOK|wxICON_EXCLAMATION);
      ...
      _("Lumimaja: Error initializing help"), wxOK | wxICON_ERROR);
#endif
    }
  }
  ...
  if (!cmd_closed && !cmd_silent && !cmd_minimized) {
    ...
    //returnValue = m_frame->Load(initWindow->GetPassword());
    if (returnValue != 0) {
      // XXX
    }
  } else {
...
{
  static wxString DOMAIN_(L"lumimaja");
...
void PwsafeApp::SaveFrameCoords(void)
{
  if (m_frame->IsMaximized()) {
    ...
    //if we save coords when minimized/hidden, lumimaja could be hidden or off
