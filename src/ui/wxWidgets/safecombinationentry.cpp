...
int CSafeCombinationEntry::ProcessPhrase()
{
  wxString err;
  ...
  m_core.SetCurFile(L"");
  wxMessageDialog errdg(this, err,
                      _("Error"), wxOK | wxICON_EXCLAMATION);
  errdg.ShowModal();
...
void CSafeCombinationEntry::OnNewDbClick( wxCommandEvent& /* evt */ )
{
  ...
  stringT v3FileName = PWSUtil::GetNewFileName(tostdstring(cf), wxT("lumi3"));
  ...
  if ((rc = m_core.WriteCurFile()) == PWScore::SUCCESS) {
