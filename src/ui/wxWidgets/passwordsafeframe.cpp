...
////@begin PasswordSafeFrame content construction
  ...
  itemMenu79->Append(ID_VISITWEBSITE, _("Visit Lumimaja &website..."), _T(""), wxITEM_NORMAL);
...
int PasswordSafeFrame::Load(const StringX &passwd)
{
  ...
  fprintf(stderr, "PasswordSafeFrame::Load %d\n", status);
...
int PasswordSafeFrame::Save(SaveType st /* = ST_INVALID*/)
{
  ...
  switch (m_core.GetReadFileVersion()) {
    ...
    case PWSfile::NEWFILE:
    {
      ...
      wxString msg( wxString::Format(_("The original database, \"%ls\", is in pre-3.0 format. It will be unchanged.\nYour changes will be written as \"%ls\" in the new format, which is unusable by old versions of Lumimaja. To save your changes in the old format, use the \"File->Export To-> Old (1.x or 2) format\" command."),
...
#if 0
      m_titlebar = PWSUtil::NormalizeTTT(wxT("Lumimaja - ") +
  ...
  fprintf(stderr, "PasswordSafeFrame::Save WriteCurFile() rc=%d\n", rc);
...
int PasswordSafeFrame::DoOpen(const wxString& title)
{
  ...
  wxFileDialog fd(this, title, dir.c_str(), wxT("lumimaja.lumi3"),
...
int PasswordSafeFrame::Open(const wxString &fname)
{
  ...
  fprintf(stderr, "passwordsafeframe pwdprompt\n");
  ...
  if (pwdprompt.ShowModal() == wxID_OK) {
    ...
    fprintf(stderr, "passwordsafeframe before load\n");
  ...
  m_titlebar = PWSUtil::NormalizeTTT(_T("Lumimaja - ") +
                                     m_core.GetCurFile()).c_str();
...
int PasswordSafeFrame::SaveAs()
{
  if (m_core.GetReadFileVersion() != PWSfile::VCURRENT &&
    ...
    if (wxMessageBox( wxString::Format(_("The original database, '%ls', is in pre-3.0 format. The data will now be written in the new format, which is unusable by old versions of Lumimaja. To save the data in the old format, use the 'File->Export To-> Old (1.x or 2) format' command."),
  ...
  if(cf.empty()) {
    cf = wxT("lumimaja"); // reasonable default for first time user
  }
  ...
  wxFileDialog fd(this, title, dir, filename.GetFullName(),
                  _("Lumimaja Databases (*.lumi3; *.dat)|*.lumi3;*.dat|All files (*.*; *)|*.*;*"),
...
static void DisplayFileWriteError(int rc, const StringX &fname)
{
  ...
  wxMessageDialog err(NULL, cs_temp, cs_title, wxOK | wxICON_ERROR);
  err.ShowModal();
}
...
int PasswordSafeFrame::New()
{
  ...
  SetLabel(PWSUtil::NormalizeTTT(wxT("Lumimaja - ") + cs_newfile).c_str());
...
int PasswordSafeFrame::NewFile(StringX &fname)
{
  ...
  wxString cf(wxT("lumimaja")); // reasonable default for first time user
  ...
  while (1) {
    wxFileDialog fd(static_cast<wxWindow *>(this), cs_text, dir, v3FileName,
                    _("lumi3 files (*.lumi3)|*.lumi3|All files(*.*; *)|*.*;*"),
...
void PasswordSafeFrame::OnImportXML(wxCommandEvent& evt)
{
  ...
  if (!XSDFilename.FileExists()) {
    ...
    wxMessageBox(wxString::Format(_("Can't find XML Schema Definition file (%ls) in your Lumimaja Application Directory.\nPlease copy it from your installation file, or re-install Lumimaja."), filepath.c_str()),
...
{
  wxLaunchDefaultBrowser("https://samifar.in");
}
