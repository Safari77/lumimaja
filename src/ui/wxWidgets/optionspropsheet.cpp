...
#include "pwsafeapp.h" // for set/get hashMemKiB / hashPasses
...
////@begin COptions content construction
  ...
  wxCheckBox* itemCheckBox31 = new wxCheckBox( itemPanel29, ID_CHECKBOX12, _("Always keep Lumimaja on top"), wxDefaultPosition, wxDefaultSize, 0 );
  ...
  itemBoxSizer97->Add(itemSlider100, 0, wxGROW|wxALL, 5);

  wxBoxSizer* itemBoxSizer100 = new wxBoxSizer(wxHORIZONTAL);
  itemBoxSizer97->Add(itemBoxSizer100, 0, wxGROW|wxALL, 5);
  wxStaticText* itemStaticText101 = new wxStaticText( itemPanel86, wxID_STATIC, _("32 MiB"), wxDefaultPosition, wxDefaultSize, 0 );
  ...
  wxStaticText* itemStaticText102 = new wxStaticText( itemPanel86, wxID_STATIC, _("32 GiB"), wxDefaultPosition, wxDefaultSize, 0 );
  itemBoxSizer100->Add(itemStaticText102, 0, wxALIGN_CENTER_VERTICAL|wxALL, 5);
  ...
  itemBoxSizer98->Add(itemSlider99, 0, wxGROW|wxALL, 5);
  ...
  wxCheckBox* itemCheckBox112 = new wxCheckBox( itemPanel104, ID_CHECKBOX31, _("Start Lumimaja at Login"), wxDefaultPosition, wxDefaultSize, 0 );
  ...
  wxStaticBox* itemStaticBoxSizer113Static = new wxStaticBox(itemPanel104, wxID_ANY, _("Recent Lumimaja Databases"));
  ...
  itemSlider99->SetValidator( wxGenericValidator(& m_hashPassesSlider) );
  itemSlider100->SetValidator( wxGenericValidator(& m_hashMemKiBSlider) );
...
void COptions::PrefsToPropSheet()
{
  ...
  uint32 hashPasses = app->GetHashPasses();
  if (hashPasses < MIN_HASH_PASSES) {
    hashPasses = MIN_HASH_PASSES;
  }
  if (hashPasses > MAX_HASH_PASSES) {
    hashPasses = MAX_HASH_PASSES;
  }
  m_hashPassesSlider = hashPasses;

  uint32 hashMemKiB = app->GetHashMemKiB();
  if (hashMemKiB <= MIN_HASH_MEM_KIB) {
    m_hashMemKiBSlider = 0;
  } else {
    const int step = MAX_HASH_MEM_KIB/100;
    m_hashMemKiBSlider = uint32(hashMemKiB/step);
  }
...
void COptions::PropSheetToPrefs()
{
  ...
  PwsafeApp *app = dynamic_cast<PwsafeApp *>(wxTheApp);
  if (m_hashPassesSlider > MAX_HASH_PASSES) {
    m_hashPassesSlider = MAX_HASH_PASSES;
  }
  if (m_hashPassesSlider < MIN_HASH_PASSES) {
    m_hashPassesSlider = MIN_HASH_PASSES;
  }
  app->SetHashPasses(m_hashPassesSlider);

  uint32 value = MIN_HASH_MEM_KIB;
  if (m_hashMemKiBSlider > 0) {
    const int step = MAX_HASH_MEM_KIB/100;
    value = uint32(m_hashMemKiBSlider*step);
  }
  app->SetHashMemKiB(value);
...
void COptions::OnSuffixCBSet( wxCommandEvent& /* evt */ )
{
  ...
  if (example.empty())
    example = wxT("lumimaja"); // XXXX get current file's basename!
